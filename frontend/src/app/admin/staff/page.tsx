...
export default function StaffAdminPage() {
  ...
  const [formHourlyWage, setFormHourlyWage] = useState(1150);
  const [formMaxWeeklyHours, setFormMaxWeeklyHours] = useState(30);
  const [formTargetWeeklyHours, setFormTargetWeeklyHours] = useState(25);
  const [formMaxConsecutiveDays, setFormMaxConsecutiveDays] = useState(4);
  ...
  const [formYtdEarnings, setFormYtdEarnings] = useState(600000);
  ...
  const handleOpenAdd = () => {
    ...
    setFormRoles(['hall']);
    setFormHourlyWage(1150);
    setFormMaxWeeklyHours(30);
    setFormTargetWeeklyHours(25);
    setFormMaxConsecutiveDays(4);
    setFormMinDays(3);
    setFormMaxDays(7);
    setFormNgStaffIds([]);
    setFormPreferredPartnerIds([]);
    setFormYtdEarnings(0);
  ...
  const handleOpenEdit = (staff: StaffMember) => {
    ...
    setFormHourlyWage(staff.hourly_wage);
    setFormMaxWeeklyHours(staff.max_weekly_hours);
    setFormTargetWeeklyHours(staff.target_weekly_hours);
    setFormMaxConsecutiveDays(staff.max_consecutive_days);
    setFormMinDays(staff.min_days_per_period ?? 2);
    setFormMaxDays(staff.max_days_per_period ?? 7);
    ...
    setFormYtdEarnings(staff.annual_earnings_ytd ?? 0);
  ...
  const handleDeleteStaff = (staffId: string) => {
    ...
    if (!target) return;
    if (!window.confirm(`「${target.name}」を削除してもよろしいですか？`)) return;
  ...
  const handleSaveForm = (e: React.FormEvent) => {
    ...
    if (formHourlyWage < 800) {
      setFormError('時給は800円以上で入力してください');
      return;
    }
    ...
    const newStaffObj: StaffMember = {
      ...
      roles: formRoles,
      hourly_wage: Number(formHourlyWage),
      max_weekly_hours: Number(formMaxWeeklyHours),
      target_weekly_hours: Number(formTargetWeeklyHours),
      max_consecutive_days: Number(formMaxConsecutiveDays),
      min_days_per_period: Number(formMinDays),
      max_days_per_period: Number(formMaxDays),
      ...
      annual_earnings_ytd: Number(formYtdEarnings),
    ...
    showToast(editingStaff ? `「${formName}」を更新しました` : `新規スタッフ「${formName}」を追加しました`);
  };
  ...
  return (
    <main className="container" style={{ paddingBottom: '3rem' }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
            在籍スタッフ {requestData.staff_members.length} 名 | 労基法制約・時給・保有ロール・相性NGペアの設定
          </p>
        </div>
      ...
      <div className="card" style={{ padding: '0', overflowX: 'auto' }}>
        <table className="modern-table" style={{ minWidth: '960px' }}>
          <thead>
            <tr>
              <th style={{ width: '180px' }}>氏名 / 属性</th>
              <th>保有ロール</th>
              <th style={{ width: '90px' }}>時給</th>
              <th style={{ width: '120px' }}>週間/連勤上限</th>
              <th style={{ width: '140px' }}>NGペア / 希望ペア</th>
              <th style={{ width: '160px' }}>年収の壁 / 累計</th>
              <th style={{ width: '110px', textAlign: 'right' }}>操作</th>
            </tr>
          </thead>
          <tbody>
            {requestData.staff_members.map((staff) => {
              ...
              return (
                <tr key={staff.id} data-testid={`staff-item-${staff.id}`}>
                  <td>
          ...
          <div className="modal-dialog" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '580px' }}>
            ...
            <form onSubmit={handleSaveForm}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                ...
                <div className="form-group">
                  <label className="form-label">満18歳未満 (労基法年少者)</label>
                  <label
                    style={{
                      display: 'flex',
                      alignItems: 'center',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div className="form-group">
                  <label className="form-label">週間最大労働時間 (h)</label>
                  <input
                    type="number"
                    className="form-input"
                ...
                <div className="form-group">
                  <label className="form-label">連続勤務上限日数</label>
                  <input
                    type="number"
                    className="form-input"
                    value={formMaxConsecutiveDays}
                    onChange={(e) => setFormMaxConsecutiveDays(Number(e.target.value))}
                    min={1}
                    max={7}
                  />
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                ...
                <div className="form-group">
                  ...
                  <input
                    type="number"
                    className="form-input"
                    value={formYtdEarnings}
                    onChange={(e) => setFormYtdEarnings(Number(e.target.value))}
                    min={0}
                    step={10000}
                  />
                </div>
